...
public class Adapter_already_yi extends BaseAdapter {
	...
	public Adapter_already_yi(Context context, List<PurchaseEntity> list) {
	...
	public View getView(final int position, View currentView, ViewGroup parent) {
		...
		if (currentView == null) {
			...
			currentView = inflater.inflate(R.layout.adapter_yipayment, null);
			...
			holderView.item = (LinearLayout) currentView.findViewById(R.id.lin_listy);
			currentView.setTag(holderView);
		} else {
			holderView = (HolderView) currentView.getTag();
		}
		PurchaseEntity sy = mlist.get(position);

		String tid = sy.Tid;
		double iv_pay = sy.Payment;
		String time = sy.Created;
		JSONArray jsonarray = sy.jsonarray1;
		
		setdata(position, holderView, sy, jsonarray);
